    ...
    parseSLT() {
      ...
      if (this.type !== acorn.tokTypes.braceL) {
        ...
        if (this.type === acorn.tokTypes.semi
          ...
          this.next();
        return {
          type: "SLTExpression", keyword,
          value: value.trim(), start,
          end: this.lastTokEnd
        };
      }
      ...
      if (this.type === acorn.tokTypes.braceL) {
        ...
        if (this.type === acorn.tokTypes.braceR &&
            typeof this.next === "function")
          this.next();
        return { type: "SLTExpression", keyword,
          body, start, end: this.lastTokEnd };
      }

      this.raise && this.raise(start,
        `Unexpected SLT keyword: ${keyword}`);
    }
  };
}
